...
const ROLE_CFG: Record<GroupRole, { label: string; cls: string }> = {
  owner:    { label: 'Owner',    cls: 'bg-secondary-container/20 text-on-secondary-fixed-variant' },
  co_admin: { label: 'Co-admin', cls: 'bg-secondary-fixed text-on-secondary-fixed-variant' },
  member:   { label: 'Member',   cls: 'bg-surface-container-highest text-on-surface-variant' },
};

// ── Group icon by role ────────────────────────────────────────

const GROUP_ICONS: Record<GroupRole, { icon: string; bg: string; text: string }> = {
  owner:    { icon: 'sprint',          bg: 'bg-primary-container', text: 'text-on-primary' },
  co_admin: { icon: 'directions_run',  bg: 'bg-secondary-container', text: 'text-on-secondary-container' },
  member:   { icon: 'directions_bike', bg: 'bg-surface-container-high', text: 'text-primary' },
};
        ...
        <div className="relative group flex-shrink-0">
          <div className="w-32 h-32 md:w-40 md:h-40 rounded-xl overflow-hidden shadow-xl bg-surface-container-high flex items-center justify-center">
            {user?.avatarUrl
              ...
              : <span className="text-4xl md:text-5xl font-black font-headline text-on-surface-variant">{initials}</span>
            }
          </div>
          {ownedCount > 0 && (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
        ...
        <aside className="lg:col-span-4 space-y-8">
          ...
          <div className="bg-surface-container-lowest rounded-xl p-6 shadow-soft space-y-6">
            <h3 className="font-headline font-bold text-lg tracking-tight">Athlete Stats</h3>
            <div className="space-y-3">
              <div className="p-4 bg-surface-container-low rounded-lg flex items-center justify-between">
                <span className="text-on-surface-variant font-medium">Active Groups</span>
                <span className="font-headline font-black text-xl text-primary">{String(activeCount).padStart(2, '0')}</span>
              </div>
              <div className="p-4 bg-surface-container-low rounded-lg flex items-center justify-between">
                <span className="text-on-surface-variant font-medium">Groups Owned</span>
                <span className="font-headline font-black text-xl text-primary">{String(ownedCount).padStart(2, '0')}</span>
              </div>
              <div className="p-4 bg-surface-container-low rounded-lg flex items-center justify-between">
                <span className="text-on-surface-variant font-medium">Account</span>
                <span className="font-headline font-black text-sm text-secondary">Active</span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-primary-container text-on-primary-container rounded-xl p-8 overflow-hidden relative">
            <div className="relative z-10">
              <h4 className="font-headline font-bold text-xl mb-2 text-white">Invite Your Friends</h4>
        ...
        <div className="lg:col-span-8 space-y-12">
          ...
          <section>
            ...
            <div className="bg-surface-container-lowest rounded-xl p-8 shadow-soft space-y-8">
              ...
              <div className="pt-4 space-y-4">
                ...
                {[
                  { label: 'Push Notifications', desc: 'Real-time alerts for event starts and group updates.', checked: true },
                  { label: 'Email Digest',        desc: 'Weekly summary of upcoming events and group activity.', checked: false },
                ].map(n => (
                  <div key={n.label} className="flex items-center justify-between">
                    ...
                    <div className="relative inline-flex items-center cursor-not-allowed">
                      <input readOnly checked={n.checked} className="sr-only peer" type="checkbox" />
          ...
          <section>
            <div className="flex justify-between items-end mb-6">
              <h2 className="font-headline font-extrabold text-2xl tracking-tighter">My Groups</h2>
              <button
                onClick={() => navigate('/dashboard')}
            ...
            {loadingGroups ? (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {[...Array(4)].map((_, i) => (
                  <div key={i} className="h-40 bg-surface-container-lowest rounded-xl animate-pulse shadow-soft" />
                ))}
              </div>
            ) : groups.length === 0 ? (
              <div className="bg-surface-container-lowest rounded-xl p-12 text-center shadow-soft">
                <span className="material-symbols-outlined text-4xl text-on-surface-variant/30 mb-3 block">group_off</span>
                <p className="font-headline font-bold text-primary mb-1">No groups yet</p>
                <button onClick={() => navigate('/dashboard')} className="text-secondary text-sm font-bold hover:underline mt-1">
                  ...
                  return (
                    ...
                    >
                      <div className="flex justify-between items-start">
                        <div className={`w-12 h-12 ${iconCfg.bg} rounded-lg flex items-center justify-center ${iconCfg.text}`}>
                          <span className="material-symbols-outlined">{iconCfg.icon}</span>
                        </div>
                        <span className={`px-3 py-1 rounded-full text-[10px] font-black tracking-widest uppercase ${role.cls}`}>
                          {role.label}
                        </span>
                      </div>
                      <div>
